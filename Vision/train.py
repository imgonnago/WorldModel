...
def save_checkpoint(model, optimizer, scheduler,epoch, train_loss, val_loss, save_path):
    checkpoint = {
        "epoch": epoch,
        "model_state_dict": model.state_dict(),
        "optimizer_state_dict": optimizer.state_dict(),
        "scheduler_state_dic" : scheduler.state_dict(),
...
def load_checkpoint(model, optimizer, scheduler, load_path, device):
...
def train_vae(
    ...
    if resume_from is not None and os.path.exists(resume_from):
        model, optimizer, start_epoch = load_checkpoint(model, optimizer, scheduler, resume_from, device)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            no_improve_count = 0
            save_checkpoint(model, optimizer, scheduler, epoch, avg_train_loss, avg_val_loss, save_path=os.path.join(checkpoint_dir, "best_checkpoint.pth"))
